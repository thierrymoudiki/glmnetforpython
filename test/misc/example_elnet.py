...
x = x.astype(dtype = np.float64, order = 'C', copy = True)
...
# w
w = np.ones([no], dtype = np.float64)
w = w.astype(dtype = np.float64, order = 'F', copy = True)    
w_r = w.ctypes.data_as(ctypes.POINTER(ctypes.c_double))
# jd
jd = np.ones([1], dtype = np.int32)
jd = jd.astype(dtype = np.int32, order = 'F', copy = True)    
jd_r = jd.ctypes.data_as(ctypes.POINTER(ctypes.c_int))
# vp
vp = np.ones([ni], dtype = np.float64)
vp = vp.astype(dtype = np.float64, order = 'F', copy = True)    
...
if cl.shape[1] < ni:
    ...
    cl = cl[:, 0:ni-1]
cl = cl.astype(dtype = np.float64, order = 'F', copy = True)    
...
# ulam
ulam   = np.zeros([1], dtype = np.float64)
ulam   = ulam.astype(dtype = np.float64, order = 'F', copy = True)    
