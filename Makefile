...
lint: ## check style with flake8
	flake8 glmnetforpython tests

coverage: ## check code coverage quickly with the default Python	
	coverage report --omit="venv/*,glmnetforpython/tests/*" --show-missing
...
build-site: docs ## export mkdocs website to a folder		
	cp -rf glmnetforpython-docs/* ../../Pro_Website/Techtonique.github.io/glmnetforpython
	find . -name '__pycache__' -exec rm -fr {} +
...
run-tests: install ## run all the tests with one command
	...
	python3 -m coverage run -m unittest discover -s glmnetforpython/tests -p "*.py"	
