...
import scipy
from glmnetclass import glmnet
...
def cvglmnet(
    ...
    **options
    ...
    #    options['offset'] = np.transpose(options['offset'])

    if len(options["weights"]) == 0:
        options["weights"] = np.ones([nobs, 1], dtype=np.float64)
    ...
    if glmfit["class"] == "multnet":
        nnz = np.zeros([len(options["lambdau"]), len(nz)])
        for i in range(len(nz)):
            nnz[:, i] = np.transpose(scipy.sum(nz[i], axis=0))
        nz = scipy.ceil(scipy.median(nnz, axis=1))
    elif glmfit["class"] == "mrelnet":
        nz = np.transpose(scipy.sum(nz[0], axis=0))
    else:
        nz = np.transpose(scipy.sum(nz, axis=0))

    if len(foldid) == 0:
        ma = scipy.tile(
            scipy.arange(nfolds), [1, int(scipy.floor(nobs / nfolds))]
        )
    ...
    if parallel != 1:
        if parallel == -1:
            ...
            num_cores = parallel
        sys.stderr.write(
            "[status]\tParallel glmnet cv with " + str(num_cores) + " cores\n"
        )
        cpredmat = joblib.Parallel(n_jobs=num_cores)(
            joblib.delayed(doCV)(
    ...
    CVerr["cvm"] = np.transpose(cvm)
    CVerr["cvsd"] = np.transpose(cvsd)
    CVerr["cvup"] = np.transpose(cvm + cvsd)
    CVerr["cvlo"] = np.transpose(cvm - cvsd)
    CVerr["nzero"] = nz
    CVerr["name"] = cvname
    ...
    CVerr["lambda_min"] = scipy.amax(
        options["lambdau"][cvm <= scipy.amin(cvm)]
    ).reshape([1])
